...
const StepRender = () => {
  ...
  const handleCalResult = () => {
    setShowResult(true)
  }

  const percentCompleted = (answeredQuestions.filter(Boolean).length / questions.length) * 100;
      ...
      <div>
        ...
        {showResult ?
          ...
          <>
            ...
            <div>
              {Array.from({ length: questions[currentQuestionIndex].difficulty === "hard" ? 3 : questions[currentQuestionIndex].difficulty === "medium" ? 2 : 1 }, (_, i) => (
                <span key={i}>⭐</span>
              ))}
            </div>
            <Question
              question={questions[currentQuestionIndex].question}
              choices={[questions[currentQuestionIndex].correct_answer, ...questions[currentQuestionIndex].incorrect_answers]}
            ...
            {showNextQuestionButton && (
              <>
                {currentQuestionIndex === 19 ? <button onClick={handleCalResult} style={{ cursor: 'pointer' }}>Show Result</button> : <button onClick={handleNextQuestionClick} style={{ cursor: 'pointer' }}>Next Question</button>}
              </>
            )}
          </>
        }
